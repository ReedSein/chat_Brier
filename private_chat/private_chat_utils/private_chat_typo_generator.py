...
class TypoGenerator:
    ...
    def __init__(
        ...
        # 错字相关可配置参数
        self.min_text_length = config.get("typo_min_text_length", 5)  # 最小文本长度
        self.min_chinese_chars = config.get("typo_min_chinese_chars", 3)  # 最小汉字数量
        self.min_message_length = config.get(
            "typo_min_message_length", 10
        )  # 触发判断的最小消息长度
        self.min_typo_count = config.get("typo_min_count", 0)  # 最少添加错字数
        self.max_typo_count = config.get("typo_max_count", 2)  # 最多添加错字数
    ...
    def _init_common_homophones(
        ...
        # 从配置中读取自定义同音字
        if config:
            custom_json = config.get("typo_homophones", "")
