...
# 🆕 v1.2.0: 标记键名，用于标识请求来自本插件（与群聊版本共用相同的标记）
    ...
    async def generate_reply(
        ...
        try:
            ...
            if prompt_mode == "override" and extra_prompt and extra_prompt.strip():
                # 覆盖模式：直接使用用户自定义提示词（仍然添加发送者强调）
                full_prompt = (
                    formatted_message + sender_emphasis + "\n\n" + extra_prompt.strip()
                )
                if DEBUG_MODE:
                    logger.info("使用覆盖模式：用户自定义提示词完全替代默认系统提示词")
            else:
                # 拼接模式（默认）：先添加发送者强调，再添加系统提示词
                full_prompt = (
                    formatted_message
                    + sender_emphasis
                    + "\n\n"
            ...
            if DEBUG_MODE:
                logger.info(f"🔧 [兼容模式] 已设置插件标记，将通过 event.request_llm() 调用 AI")
